...
class GetInvolved(toolkit.CkanCommand):
    ...
    def update_db(self):
        ...
        print(u'Attempt to update volunteering_opportunity table')
        q = text(
            "ALTER TABLE volunteering_opportunity "
            "ADD COLUMN IF NOT EXISTS email_contact varchar NOT NULL DEFAULT '';"
        )
        engine.execute(q)
